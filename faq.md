...
Instead, mediasoup launches a set of C++ child processes ([media workers](https://github.com/versatica/mediasoup/tree/master/worker)) and communicates with them by means of inter-process communication. This approach leads to a media worker design not tiled to the internals of Node.js or V8 (which change in every new release).
