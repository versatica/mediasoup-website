...
`peerConnectionOptions` | [PeerConnection::Options\*](#PeerConnectionOptions) | PeerConnection options. `peerConnectionOptions.config` can be used as a counterpart for `iceServers`, `iceTransportPolicy` and `additionalSettings` parameters from TypeScript mediasoup-client implementation. | No   |
